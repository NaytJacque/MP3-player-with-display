...
uint8_t eq = 0;
uint8_t shuffledTrack;
uint8_t normalFile = 1;
...
void setup(void) {
  ...
  //Read atmospheric noise from unconnected analog pin 0 to mix up the seed
  randomSeed(
  analogRead(A1) +
  analogRead(A2) +
  micros()
...
void loop() {
  ...
  float batteryVoltage = (adc * 5.0 / 1023.0) * 2.0;
