...
void settings() {
  ...
  if (selection == 2 && !inSideMenuSelection) {
    u8g2.setFont(soundpod_icon_pack_font);
    if (eq > 0)
      u8g2.drawGlyph(85, 50, 69); // drawing left ajustment button next to the volume
    if (eq < 5)
      u8g2.drawGlyph(100, 50, 70); // drawing right ajustment button next to the volume
  }
  u8g2.setFont(u8g2_font_glasstown_nbp_tf);
...
void player() {
 ...
 if (selection == 4 && !inSideMenuSelection) {
    u8g2.drawBox(45, 50, 40, 10);
    u8g2.setDrawColor(0);
  }
  u8g2.setFont(u8g2_font_glasstown_nbp_tf);
  u8g2.setCursor(50, 60);
  u8g2.print(F("Set to 1"));
  u8g2.setDrawColor(1);
